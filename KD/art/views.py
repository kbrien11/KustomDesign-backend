...
from .serializers import CustomUserSerializer,PictureSerializer,MatchRelationshipSerializer
...
def createUser(request):
    ...
    print(pass_hash)
    new_user = CustomUser(email=request.data['email'],password=pass_hash,user_type=request.data['user_type'],first_name=first_name,last_name=last_name,username=username,profile_image=profile_image)
    if new_user:
        new_user.save()
...
def login(request):
    ...
    if user_obj:
        ...
        if validate_password:        
            return Response({'user_type': ser.data['user_type'],'id':ser.data['id']})
        else:
...
def upload_image(request,user_pk):
    ...
    print(user_ser.data)
    if user_ser:
        new_picture  = Picture(user_pk=user_pk,image = request.data['image'],size=request.data['size'])
        if new_picture:
            new_picture.save()
...
def get_images(request):
    ...
    if picture_ser:
        return Response({"images":picture_ser.data})
    else:
        return Response({"error":"invalid user"})
...
def imagePerArtist(request,artist_id,username):
    ...
    if artist_username:
        ...
        if artistMatchesSer:
            ...
            return Response({"data":picture_list})
        else:
...
def artist_profile_picture(reqest,artist_id,artist_username):
    ...
    if artist_name:
        ...
        artist_ser = CustomUserSerializer(artist,many = False)
        if artist_ser.data:
            return Response({"data":artist_ser.data['profile_image']})
        else:
            return Response({"data":"error"})